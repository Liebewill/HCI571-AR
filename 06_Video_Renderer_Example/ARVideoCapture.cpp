...
int ARVideoCapture::init()
{
	...
	if(_useFile == false)
	{
		...
		_capture = new VideoCapture(_deviceID);
	...
	else if(_useFile == true)
	{
		...
		_capture = new VideoCapture(_file.c_str());
	}
    ...
    if(_capture->isOpened())
	{
		...
		_size = cvSize((int) _capture->get(CV_CAP_PROP_FRAME_WIDTH), (int) _capture->get(CV_CAP_PROP_FRAME_HEIGHT));

		// Query the frame rate
		_framerate = ( int ) _capture->get(CV_CAP_PROP_FPS);

		// Prepare a video image as output
		_convertedImage = cvCreateImage(_size, IPL_DEPTH_8U, 3); 
        ...
        (*_capture) >> _image;
        cvtColor(_image, _convertedImage, CV_RGBA2RGB);
