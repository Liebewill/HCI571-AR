...
void setTextureToRect(osg::Texture2D* texture, int videoWidth, int videoHeight, int textureWidth, int textureHeight)
{
	...
	sizeX = double(videoWidth)/double(textureWidth); // 0.625
	sizeY = double(videoHeight)/double(textureHeight);// 0.703
    ...
    for(int j=0; j<textureCoordGridY-1; j++) // rows
    {
        for(int i=0; i<textureCoordGridX-1; i++) // columns
        {
           ...
           double dx = 0.0;
            ...
            (*texCoordArray)[idx0 + 2] = osg::Vec3(cx0 + dx, cy0, 0.0);
            (*texCoordArray)[idx0 + 3] = osg::Vec3(cx0 + deltaX + dx, cy0, 0.0);
            (*texCoordArray)[idx0 + 0] = osg::Vec3(cx0 + deltaX + dx, cy0 + deltaY, 0.0);
            (*texCoordArray)[idx0 + 1] = osg::Vec3(cx0 + dx, cy0 + deltaY, 0.0);
...
osg::Group* createSzene(void)
{
	...
	myVideoCaptureDevice = new ARVideoCapture(0);
